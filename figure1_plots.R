...
toc_stack$ord_site <- factor(toc_stack$site, levels=c("Lake Athabasca", "Burnt Lakes", "Saline Lake", "Lac La Biche"))
dw_stack$ord_site <- factor(dw_stack$site, levels=c("Lake Athabasca", "Burnt Lakes", "Saline Lake", "Lac La Biche"))
...
figure1_toc <-ggplot(data= toc_stack, aes(y = year, x = value, color=variable)) +
  ...
  facet_wrap(~ord_site, strip.position="bottom", ncol=1) +
...
figure1_dw <-ggplot(data= dw_stack, aes(y = year, x = value, color=variable)) +
  ...
  geom_point(size=1) +
  scale_color_manual(values=c("navy", "#56B4E9"), name = "Group", labels = c("Total PAC", "Total APAC"))+
  facet_wrap(~ord_site, strip.position="bottom") +
