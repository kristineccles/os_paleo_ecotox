...
df7_stack$ord_site <- factor(df7_stack$site, levels=c("Lake Athabasca", "Burnt Lakes", "Saline Lake", "Lac La Biche"))
...
figure7 <-ggplot(data= df7_stack, aes(y = year, x = value)) +
  ...
  facet_grid(ord_site~variable, scales="free_x", switch="x")+
  ylab("Year")+
